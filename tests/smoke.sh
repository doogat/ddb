...
TFU_PORT=$((19200 + (RANDOM % 800)))
$DDB serve --port "$TFU_PORT" &
TFU_PID=$!
TOKEN=$(cat ~/.config/ddb/token 2>/dev/null || echo '')
for i in $(seq 1 20); do
  if curl -sf "http://127.0.0.1:$TFU_PORT/graphql" \
...
tfu_gql() {
  curl -sf "http://127.0.0.1:$TFU_PORT/graphql" \
    -H "Authorization: Bearer $TOKEN" \
    -H "Content-Type: application/json" \
    -d "$1"
}
# Create typedef with VARCHAR(200) column
tfu_gql '{"query":"mutation { executeSql(sql: \"CREATE TABLE tfubookmark (url VARCHAR(200))\") { message } }"}' | grep -q "table tfubookmark created"
# Create a typed doogat via SQL INSERT
TFU_ID=$(tfu_gql '{"query":"mutation { executeSql(sql: \"INSERT INTO tfubookmark (title, url) VALUES (\\\"TFU Test\\\", \\\"https://old.com\\\")\") { message } }"}' | sed -n 's/.*"message":"\([^"]*\)".*/\1/p')
echo "$TFU_ID" | grep -qE "^[0-9]{14}$"
# Verify initial materialized row
tfu_gql "{\"query\":\"mutation { executeSql(sql: \\\"SELECT url FROM tfubookmark WHERE id = '$TFU_ID'\\\", format: \\\"objects\\\") { rows } }\"}" | grep -q "https://old.com"
# updateDoogat with fields to change url
TFU_UPD=$(tfu_gql "{\"query\":\"mutation { updateDoogat(input: { id: \\\"$TFU_ID\\\", fields: \\\"{\\\\\\\"url\\\\\\\":\\\\\\\"https://updated.com\\\\\\\"}\\\" }) { id } }\"}")
echo "$TFU_UPD" | grep -q "$TFU_ID"
# Verify via SQL SELECT that materialized row has updated url
tfu_gql "{\"query\":\"mutation { executeSql(sql: \\\"SELECT url FROM tfubookmark WHERE id = '$TFU_ID'\\\", format: \\\"objects\\\") { rows } }\"}" | grep -q "https://updated.com"
pass "typed field update via GraphQL"
# updateDoogat with unsetFields to remove url
TFU_UNSET=$(tfu_gql "{\"query\":\"mutation { updateDoogat(input: { id: \\\"$TFU_ID\\\", unsetFields: [\\\"url\\\"] }) { id } }\"}")
echo "$TFU_UNSET" | grep -q "$TFU_ID"
# Verify url is gone (NULL)
TFU_AFTER=$(tfu_gql "{\"query\":\"mutation { executeSql(sql: \\\"SELECT url FROM tfubookmark WHERE id = '$TFU_ID'\\\", format: \\\"objects\\\") { rows } }\"}")
if echo "$TFU_AFTER" | grep -q "https://"; then
  echo "FAIL: url should be unset after unsetFields" >&2; kill "$TFU_PID" 2>/dev/null; exit 1
fi
pass "typed field unset via GraphQL"
# Delete the doogat and verify materialized row is gone
tfu_gql "{\"query\":\"mutation { deleteDoogat(id: \\\"$TFU_ID\\\") }\"}" >/dev/null
TFU_COUNT=$(tfu_gql "{\"query\":\"mutation { executeSql(sql: \\\"SELECT COUNT(*) FROM tfubookmark WHERE id = '$TFU_ID'\\\") { rows } }\"}")
echo "$TFU_COUNT" | grep -q '"0"'
pass "typed field delete cleans materialized row"
# Clean up
tfu_gql '{"query":"mutation { executeSql(sql: \"DROP TABLE tfubookmark CASCADE\") { message } }"}' >/dev/null
kill "$TFU_PID" 2>/dev/null
wait "$TFU_PID" 2>/dev/null || true
