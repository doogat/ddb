mod alter_column_type;
mod alter_singleton;
mod alter_table_rename;
mod attachments;
mod bundle_import_conflict_recovery;
mod cascade_delete;
mod checkboxes;
mod cli_create_fk_freshness;
mod cli_create_unregistered_type;
mod cli_update_app_error;
mod collision;
mod common;
mod concurrent_writes;
mod conformance;
mod cross_transport_error;
mod date_default;
mod delete_backlinks;
mod discover;
mod fix;
mod folder_namespace;
mod help;
mod hyphenated_types;
mod inline_zone;
mod integration_alter_rename;
mod integration_binary_lww;
mod integration_bundle;
mod integration_compact_stats;
mod integration_crdt_conflicts;
mod integration_error_codes;
mod integration_graphql_batch;
mod integration_graphql_jink;
mod integration_graphql_refs;
mod integration_graphql_server;
mod integration_hyphenated_type;
mod integration_id_poison;
mod integration_junction_sync;
mod integration_nosql_cli;
mod integration_orderby_base_fields;
mod integration_rest_pgwire;
mod integration_singleton_flow;
mod integration_singleton_parity;
mod integration_singleton_races;
mod integration_sql_no_match;
mod integration_sql_writes;
mod integration_sync;
mod integration_tags_filter;
mod integration_typed_write_cli;
mod integration_typed_write_polish;
mod integration_update_bin;
mod integration_warnings_update;
mod integration_write_lock_races;
mod integration_write_recovery;
mod junction_tables;
mod link_kinds;
mod multi_device;
mod path_navigation;
mod pgwire;
mod pgwire_singleton;
mod poison_file_reindex;
mod read_pool;
mod reference_raw_id;
mod relation_resolution;
mod rename;
mod rest;
mod schema_apply;
mod search_in_filter;
mod search_junction_filter;
mod search_links;
mod sequence;
mod serve;
mod server_mutations;
mod set_flag;
mod singleton_cross_process_create;
mod singleton_cross_process_update;
mod singleton_cross_process_upsert;
mod smoke_core;
mod smoke_resilience;
mod smoke_search;
mod smoke_sql;
mod sql_expressions_test;
mod sql_lifecycle;
mod subscriptions;
mod sync_crdt;
mod tag_entries;
mod tags;
mod title_template_references;
mod typed_field_mutations;
mod typed_where_filter;
mod upsert;
