...
/// Helper: directly commit a zettel file to a node's repo (bypasses `zdb create`
/// so we can force a specific ID).
fn commit_zettel(node: &std::path::Path, id: &str, title: &str, body: &str) {
    let content = format!(
        "---\nid: {id}\ntitle: {title}\ndate: 2026-01-01\n---\n{body}\n"
    );
    ...
    std::process::Command::new("git")
        .current_dir(node)
        .args(["commit", "-m", &format!("add zettel {id}")])
        .output()
        .unwrap();
}

/// List all zettel files (excluding _typedef) in a node's zettelkasten directory.
...
/// Read a zettel file directly from disk.
fn read_zettel_raw(node: &std::path::Path, filename: &str) -> String {
    let path = node.join("zettelkasten").join(filename);
    std::fs::read_to_string(path).unwrap()
}

// ── Test: add-add collision — both zettels survive ──────────────────

#[test]
fn add_add_collision_both_zettels_survive() {
    let setup = MultiNodeSetup::new(2);

    let colliding_id = "20260101120000";

    // Node 0 creates a zettel with the colliding ID via direct file commit
    commit_zettel(&setup.nodes[0], colliding_id, "From Node 0", "Body from node zero");
    MultiNodeSetup::push(&setup.nodes[0]);

    // Node 1 creates a zettel with the SAME ID (before syncing)
    commit_zettel(&setup.nodes[1], colliding_id, "From Node 1", "Body from node one");
    ...
    // After sync, node 1 should have TWO zettel files
    let files = list_zettels(&setup.nodes[1]);
    assert_eq!(
        files.len(),
        2,
        "expected 2 zettels after collision resolution, got: {files:?}"
    );
    ...
    let other_file = files
        ...
        .expect("should have a second zettel file");
    let new_id = other_file.trim_end_matches(".md");
    assert_ne!(new_id, colliding_id, "loser should have a new ID");

    // Both zettels should preserve their original content
    let winner_content = read_zettel_raw(&setup.nodes[1], &format!("{colliding_id}.md"));
    let loser_content = read_zettel_raw(&setup.nodes[1], other_file);
    ...
    let files_0 = list_zettels(&setup.nodes[0]);
    assert_eq!(
        files_0, files,
        "both nodes should converge to same zettel set"
    );
}
...
fn add_add_collision_link_rewritten() {
    ...
    // Node 0 creates the colliding zettel AND a third zettel that links to it
    commit_zettel(
        &setup.nodes[0],
        colliding_id,
        "Target Note",
    ...
    // Create a linking zettel on node 0 (using a different ID)
    let linker_id = "20260101120100";
    let linker_content = format!(
        "---\nid: {linker_id}\ntitle: Linker Note\ndate: 2026-01-01\n---\nSee [[{colliding_id}]] for details.\n"
    );
    let linker_path = format!("zettelkasten/{linker_id}.md");
    ...
    std::process::Command::new("git")
        .current_dir(&setup.nodes[0])
        .args(["commit", "-m", "add linker zettel"])
        .output()
        .unwrap();

    MultiNodeSetup::push(&setup.nodes[0]);

    // Node 1 creates a zettel with the SAME colliding ID (before syncing)
    commit_zettel(
        &setup.nodes[1],
        colliding_id,
        "Collider Note",
    ...
    let files = list_zettels(&setup.nodes[1]);
    assert_eq!(files.len(), 3, "should have 3 zettels: {files:?}");
    ...
    let new_id_file = files
        ...
        .expect("should have a reassigned zettel file");
    let _new_id = new_id_file.trim_end_matches(".md");

    // The linker zettel's wikilink should now point to the new ID
    ...
    //
    // Since "theirs" (remote / node 0) wins on tie, the local zettel (node 1)
    ...
    let linker_on_1 = read_zettel_raw(&setup.nodes[1], &format!("{linker_id}.md"));

    // The linker should reference either the original colliding_id (winner) or
    // the new_id (if node 0's zettel was the loser). Since theirs wins on HLC
    // tie, node 0's zettel (theirs/remote) keeps the original ID.
    assert!(
    ...
    // Create a setup where local zettel links to the colliding ID, and it's the
    // loser — the link should be rewritten.
    let setup2 = MultiNodeSetup::new(2);

    // Node 0 creates a zettel with the colliding ID
    commit_zettel(
        &setup2.nodes[0],
        colliding_id,
        "Remote Note",
    ...
    // Node 1 creates the same-ID zettel AND a linker to it
    commit_zettel(
        &setup2.nodes[1],
        colliding_id,
        "Local Note",
    ...
    let local_linker_path = format!("zettelkasten/{local_linker_id}.md");
    ...
    // Sync — local zettel is the loser (theirs/remote wins on tie)
    ZdbTestRepo::zdb_at(&setup2.nodes[1])
        .arg("sync")
        .assert()
        .success()
        .stdout(predicate::str::contains("collisions reassigned: 1"));
