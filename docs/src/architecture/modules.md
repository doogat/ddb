...
traits (depends: error, types, rusqlite — defines DoogatSource, DoogatStore,
  ...
  │      │
  │      ├──> sql_engine (depends: error, types, parser, traits
  │      │                — accepts &dyn SqlBackend + &dyn DoogatStore;
  │      │                  uses indexer::materialize utilities)
  │      │
...
| `indexer` | SQLite FTS5 index (directory module: `mod.rs` core CRUD/schema/savepoints, `search.rs` FTS5 search/tag queries, `filter.rs` search filter/negation SQL building, `cascade.rs` delete schema/reference scans, `rebuild.rs` rebuild/reindex/staleness, `graph.rs` backlinks/discovery/sequences, `resolve.rs` path/alias/wikilink resolution, `materialize.rs` schema inference/table materialization); implements DoogatIndex | rusqlite |
| `cascade_delete` | Shared read-only cascade planner and composed reference edits for service and SQL deletion | traits, parser, types |
| `sql_engine` | SQL DDL/DML → doogat CRUD, _typedef management (directory module: `mod.rs` dispatch, `ddl.rs` CREATE/ALTER/DROP, `dml.rs` INSERT/UPDATE and DELETE dispatch, `delete.rs` cascade-delete execution, `junction.rs` junction tables, `builders.rs` doogat/schema building, `helpers.rs` SQL parsing utilities, `transaction.rs` BEGIN/COMMIT/ROLLBACK) | sqlparser, rusqlite |
