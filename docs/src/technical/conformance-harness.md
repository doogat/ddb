...
- **Cross-interface error-code equality** — deferred. The `validation_error` workflow does not yet produce a real validation error, so the current cross-driver test only checks that both drivers run the fixture to completion. When the fixture uses input ddb rejects, compare the returned `code` / `message` / `context` with `DiffClass::Match`.
- **`validation_error` fixture trigger** — deferred. The fixture's `CreateDoogat { title: "" }` step does not actually trigger validation in ddb today: both `CliDriver` and `GraphqlDriver` accept the empty title and return `Ok` after generating a fallback title. **SC4** ("Structured errors can be compared across at least two interfaces") awaits either a fixture input ddb genuinely rejects or validation that rejects empty titles when no template is set.
