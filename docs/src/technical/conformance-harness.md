...
- **`validation_error` fixture trigger** — deferred. The fixture's `CreateDoogat { title: "" }` step does not actually trigger validation in ddb today: both `CliDriver` and `GraphqlDriver` accept the empty title and return `Ok` (a fallback title gets generated). The workflow therefore has no live cross-driver assertion of error-shape equivalence — only a sanity check that both drivers produce one result per step. **SC4** ("Structured errors can be compared across at least two interfaces") awaits one of: (a) the fixture switching to input ddb genuinely rejects (e.g. a non-existent type reference, malformed primary key, or a schema-validated typedef field when typed-doogat validation lands), or (b) ddb adding validation that rejects empty titles when no template is set. The `validation_error` fixture stays in `workflows.rs` as the contract placeholder. Audit trail: cycle 1 review F3 (`dev/local/reviews/00148-conformance-harness-v1-review-01.md`) surfaced the vacuous-pass gap; cycle 1 [D1] Task 4 added `Err(_)` per-driver assertions but had to `#[ignore]` them after empirically discovering the empty-title input does not trigger validation; cycle 2 review-02 (`dev/local/reviews/00148-conformance-harness-v1-review-02.md`) flagged the `#[ignore]`'d tests as masking the gap; cycle 2 [D2] Task 16 deleted those dead tests and consolidated the deferral here.
