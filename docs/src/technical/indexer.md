...
3. **Materialized columns**: introspects candidate type tables via `PRAGMA table_info`. When `types` filter is set, only those tables are checked. When a field exists in multiple type tables, subqueries are UNIONed. For `Eq` and `In`, compares the raw column value directly. For `Contains` on a REFERENCES-backed column, resolution priority is:
   - (a') **`_id`-suffix-strip** (ddb#15 follow-up): if the query token `<col>` does NOT directly name a column on any candidate table but `<col>_id` does, route through the `_id` form via the FK routes in (4) below. The Self-route shape covers the case where the type table itself carries `<col>_id`; Auto/User-junction shapes cover the cross-table case.
   - (a) if a same-named typedef table exists (convention: `<col>` matches the referenced typedef name), resolve `<val>` against that table's *search-key column* with LIKE — defaults to `title`, overridable via `ALTER TABLE <typedef> SET SEARCH KEY <col>` (PRD 00133, ddb#15 follow-up). When the override is set, the resolver substring-matches against the chosen column (e.g. `fqn`, `slug`) and `category=work.portals` hits the row whose `fqn="work.portals"` regardless of its `title`.
