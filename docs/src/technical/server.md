...
**`executeBatch(statements: [...])` is the server's atomic unit for DML within one request.** Without explicit transaction control, multiple statements use an implicit transaction. An explicit `BEGIN` must finish with `COMMIT` or `ROLLBACK` in the same batch. `CREATE TABLE` and supported column/typedef alterations join an active transaction. `DROP TABLE` and `ALTER TABLE ... RENAME TO` still write Git immediately; keep them out of batches that require rollback. See [SQL transaction limits](./sql-engine.md#transactions).
...
**Inlined target columns** — each column of `T` appears as a field (e.g. `title: StringFilter`), so `category: { title: { eq: "X" } }` works directly. A target column whose name matches any scalar/id comparison operator (`eq`, `neq`, `in`, `notIn`, `nin`, `isNull`, `gt`, `gte`, `lt`, `lte`, `contains`, `startsWith`) or any quantifier (`some`, `none`, `every`) is NOT inlined; reach it via `some: { ... }`. Only columns DECLARED on the target typedef are inlined — the base doogat fields `id`, `title`, and `date` are NOT auto-inlined. A target typedef without a declared `title` column will reject `category: { title: ... }` with a GraphQL schema-validation error. Declare the column on the target typedef (e.g. `ALTER TABLE category ADD COLUMN title TEXT`) or use `some: { title: { eq: ... } }` as a fallback if you cannot modify the typedef.

**Reverse / junction membership** — for every type `R` that REFERENCES the current type, a reverse field typed `{R}MembershipFilter` is added, exposing ONLY `some`/`none`/`every: {R}Where`. The reverse field name defaults to the pluralized referencing-type name (e.g. type `link` referencing `category` ⇒ a `links` field on `CategoryWhere`); on collision it falls back to `{referencingType}_{column}`; if it still collides it is omitted (server logs a warning). Self-referencing typedefs (a REFERENCES column pointing at the same type, e.g. `parent TEXT REFERENCES project`) are excluded from reverse-relation discovery by design — this is a deliberate scope decision, independent of any field-name collision. Forward relation filtering on a self-reference still works normally.
