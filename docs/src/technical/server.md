...
**Inlined target columns** — each column of `T` appears as a field (e.g. `title: StringFilter`), so `category: { title: { eq: "X" } }` works directly. A target column whose name collides with a reserved op (`eq`/`in`/`notIn`/`nin`/`isNull`/`some`/`none`/`every`) is NOT inlined; reach it via `some: { ... }`. Only columns DECLARED on the target typedef are inlined — the base doogat fields `id`, `title`, and `date` are NOT auto-inlined. A target typedef without a declared `title` column will reject `category: { title: ... }` with a GraphQL schema-validation error. Declare the column on the target typedef (e.g. `ALTER TABLE category ADD COLUMN title TEXT`) or use `some: { title: { eq: ... } }` as a fallback if you cannot modify the typedef.

**Reverse / junction membership** — for every type `R` that REFERENCES the current type, a reverse field typed `{R}MembershipFilter` is added, exposing ONLY `some`/`none`/`every: {R}Where`. The reverse field name defaults to the pluralized referencing-type name (e.g. type `link` referencing `category` ⇒ a `links` field on `CategoryWhere`); on collision it falls back to `{referencingType}_{column}`; if it still collides it is omitted (server logs a warning). Self-referencing typedefs (a REFERENCES column pointing at the same type, e.g. `parent TEXT REFERENCES project`) do NOT get a reverse membership field — the name would collide with the forward field. Forward relation filtering on a self-reference still works normally.
