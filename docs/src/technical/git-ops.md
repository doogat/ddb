...
- **Write lock hook** (required, no default): `with_write_lock(f)` runs `f` under the backend's repo-scoped write lock so the service layer can put a single-row read → modify → commit inside one critical section (see § "Write serialization" below for the contract)
...
No `git2` types appear in the trait signature. All OIDs are passed as `&str` hex strings. `with_write_lock` is generic (`Self: Sized`) so the trait stays dyn-compatible.

**Backend compatibility (FT-5)**: `with_write_lock` has no default on purpose — an unlocked or no-op default would silently reintroduce the lost-update hazard for any implementor who forgot to override it, so the compile break is the intended, safer failure. Anyone implementing `GitBackend` outside this crate must add it and honor the contract below (re-entrant per instance, cross-process exclusion with a bounded loud timeout, innermost resource, backend subprocess work deferred until release).
...
- **Merge/sync writes** (`ddb-core/src/git_ops/merge.rs`): the merge write path also runs inside `with_write_lock` — `commit_merge`, and `merge_remote`'s normal 3-way merge commit + forced checkout (`perform_normal_merge`) and fast-forward `set_target` + forced checkout. The lock covers all three write sections. As of PRD 00200 neither merge path builds its tree from a hand-staged `fresh_index()`: `commit_merge` rebuilds the conflicted-merge tree from libgit2's `merge_commits` result (the same construction the normal 3-way merge uses) and overlays the resolved blobs onto the conflicting paths, and the fast-forward path stages no tree (it moves the ref and force-checks-out). See § "Merge" → "Conflicted-Merge Commit (Three-Way)" for the mechanism. The `git commit-graph write` is relocated outside the locked section so its subprocess cannot stall a contended writer under the acquire timeout. Bundle import now runs through the same locked path too — `merge_bundle_and_resolve` calls `merge_remote` + `apply_merge_result` instead of shelling out to `git merge` (00168) — so every git write holds the lock.
...
- **Ordering**: the write lock is the innermost resource. SINGLETON create/upsert paths take a SQLite `BEGIN IMMEDIATE` transaction first, then commit (SQLite-outer, git-inner); no path takes the git lock and then opens a SQLite immediate transaction, so the two cannot deadlock. Code inside the write-lock hook must not rebuild the index, open a transaction, spawn a subprocess, or touch the network.
- **Single-row update boundary (FT-5, hazard H5)**: locking each commit is not enough for `ddb update <id> --set` — `DoogatService::update_doogat_parsed` used to read the stored file from HEAD *outside* the lock, so two writers on the same doogat both read base B, each committed its own field on B, and the second commit silently dropped the first's field (whole-file last-writer-wins). The service now runs the whole read → modify → commit of that one file inside `GitBackend::with_write_lock`: the stored file is re-read from HEAD under the lock, the SINGLETON/UNIQUE/FK checks run (SQLite reads only; transaction acquisition happens before the Git lock), the file is committed, and only then is the lock released. Reindexing, materialization and the NoSQL mirror update run *after* release, and the index stores that update's own commit as its indexed HEAD — never an unrelated later HEAD. The transaction decision (does this update need `BEGIN IMMEDIATE`?) is derived *before* the lock from a preview read; if a concurrent retype or `CREATE TABLE` raises that requirement between the preview and the locked read, the update refuses with a retryable `Conflict` rather than running a SINGLETON/UNIQUE check outside the transaction it needs. Batch update, SQL `UPDATE` and the raw FFI replacement path are unchanged.
- **Deferred subprocess work (FT-5)**: commit-graph rewrites and write-threshold maintenance run after the outermost write lock is released. `write_commit_graph` called under the lock (at any nesting depth, from any commit path) records the request instead of spawning `git commit-graph write`; the counting commit paths (`commit_file`, `commit_files`, `commit_binary_file`) record one pending `maintenance::check_write_threshold` per commit instead of running it inline. The outermost `with_write_lock` drains both *after* release, whether the locked body succeeded or failed: one commit-graph rewrite (coalesced), then one threshold check per commit, so the session counter advances exactly as it did inline and a threshold of N still fires on the Nth commit. `set_skip_commit_graph(true)` suppresses the deferred rewrite the same way it suppresses the inline one. Calls made outside any lock (sync's explicit post-batch `write_commit_graph`, the merge paths' post-lock rewrite) run immediately, as before.
