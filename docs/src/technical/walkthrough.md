...
`sql_engine/delete.rs` and `service/delete.rs` share the crate-private `cascade_delete` planner and reference-edit composition. Planning traverses CASCADE descendants, rejects active-path cycles and RESTRICT blockers, and deduplicates completed nodes before writes begin. `indexer/cascade.rs` supplies schema and reference queries. Execution removes junctions before index rows, applies composed edits to surviving references, and sends all file changes through the write-locked `commit_batch`. The index mutations run inside a savepoint; SQL transactions append pending files only after the whole delete succeeds.
