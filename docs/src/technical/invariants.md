...
**Current state**: closed by **00169** (poison-file-reindex-resilience). `batch_index_changes` now collects each file's read/parse failure as a `ConsistencyWarning` (`UnreadableFile` for the read stage, `MalformedYaml` for the parse stage) and skips that path instead of aborting the batch — the same collect-and-skip shape the full rebuild's `parallel_parse` already used — so one stray file no longer turns every `ensure_fresh`-gated read *and* write into an error. The old fatal behavior survives as an opt-in: a `strict` flag threaded through `incremental_reindex` → `batch_index_changes` (always `false` from the implicit/background path), reached via `DoogatService::reindex_strict()` and the `ddb reindex --strict` CLI flag. Skips are never silent — every skipped file is logged with its path, and the verb envelope carries one summarized `REINDEX_SKIPPED_FILES` `AppWarning` (first file + `(+N more, see ddb doctor)`) surfaced on CLI stderr, the GraphQL warnings extension, the REST `warnings` field, and FFI's `RebuildWarningRecord`. The parser also caps frontmatter size at the parse boundary, so an oversized synced file degrades into the same lenient skip rather than a fatal error or an unbounded parse. One follow-up remains: the per-file quarantine *listing* surface is **00181** (doctor); until it ships, the full list lives only in the logs.
