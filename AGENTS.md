...
- **Every git write holds the repo write lock — merges, sync, and bundle import included.** All commit/delete/rename/merge/checkout-moving paths must run inside the repo-scoped advisory write lock and build trees from fresh state. Never use the cached `repo.index()` for a write; never add a new write path that skips the lock. (Lock shipped for the 8 CRUD paths: 00162, done. Delete-path `fresh_index()` and the merge/sync lock + fresh merge staging shipped 2026-07-16 as 00163. Bundle import's merge leg now runs through the same write-locked merge engine (`merge_remote` + `apply_merge_result` + the new write-locked `delete_remote_ref`): 00168, done. Its unbundle/fetch object-preparation leg (`unbundle_git_objects`) still runs unlocked before the merge — known gap, does not move `HEAD` or the working tree.)
