...
mod tests {
    ...
    fn read_files_batch_partial_errors() {
        let (_dir, repo) = temp_repo();
        repo.commit_file("ddb/a.md", "content a", "add a")
            .unwrap();
        let paths = vec![
            "ddb/a.md".to_string(),
            "ddb/missing.md".to_string(),
        ];
        let batch = repo.read_files_batch(&paths).unwrap();
    ...
    fn list_doogats_finds_md_files() {
        ...
        repo.commit_file("ddb/sub/b.md", "b", "add b")
            .unwrap();
    ...
    fn commit_batch_writes_and_deletes() {
        ...
        .unwrap();
        assert_eq!(
            repo.read_file("ddb/new.md").unwrap(),
            "new content"
        );
    ...
    fn symlink_read_rejected() {
        ...
        repo.commit_file("ddb/real.md", "content", "add")
            .unwrap();
    ...
    fn normal_path_accepted() {
        let (_dir, repo) = temp_repo();
        repo.commit_file("ddb/normal.md", "ok", "add")
            .unwrap();
    ...
    fn symlink_write_rejected() {
        let (dir, repo) = temp_repo();
        repo.commit_file("ddb/real.md", "original", "add")
            .unwrap();
        let link = dir.path().join("ddb/link.md");
        std::os::unix::fs::symlink(dir.path().join("ddb/real.md"), &link).unwrap();
        let err = repo
    ...
    fn rename_file_moves_and_commits() {
        ...
        assert!(dir
            .path()
            .join("ddb/contact/20260301120000.md")
            .exists());
        let content =
            std::fs::read_to_string(dir.path().join("ddb/contact/20260301120000.md"))
                .unwrap();
        assert_eq!(content, "hello");
    }
