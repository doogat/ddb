...
- **sync**: deterministic CRDT conflict resolution (stable actor IDs) — merges now converge to the same result on every device. Post-fix `fm_crdt_bytes` differ from pre-fix bytes but are derived/rebuildable, so there is NO migration. (PRD 00165)
