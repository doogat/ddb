...
- **service**: FFI raw-Markdown surfaces `create_doogat_raw` and `update_doogat_raw` now check `REFERENCES` targets exist and reject `allowed_values` (ENUM) violations before commit, matching the typed-create path. Previously the raw path let dangling FK ids through entirely (no FK enforcement in materialized tables) and surfaced ENUM violations as opaque `DoogatError::Sql("CHECK constraint failed: ...")` errors from the SQLite layer. Unregistered types still bypass validation (raw FFI contract preserved). (PRD 00134 batch-end follow-up)
