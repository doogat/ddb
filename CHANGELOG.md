...
- **search**: accept in-query field filter syntax (e.g. `tag=rust`, `category=work.dev`) — `search()` now agrees with `normalizeSearchQuery()` on what is a valid query and routes in-query filters through the same filter SQL as the `tag`/`where` arguments (#6)
- **search**: return `invalid search query: <q>` for all malformed input (bare wildcards `*`/`**`/`.*`, unbalanced parens, bare operators) instead of surfacing as `internal error`; SQLite backend errors on the search path now default-classify as `BadRequest` with genuine backend failures (corrupt DB, disk full, etc.) still surfaced as internal errors (#9)
