...
- **server**: GraphQL error envelopes now actually carry `extensions.code` (and the structured-context fields documented in v0.2.5) on typed-mutation failures. Two regressions were stacked: the service-layer `batch_create` path flattened structured errors to `DoogatError::Validation(String)` at the cross-batch (`validation.rs`) and intra-batch (`crud.rs`) conflict sites — both now return `DoogatError::unique_violation` instead — and the resolver-layer `to_server_error` returned `async_graphql::ServerError`, which then went through async-graphql v7's blanket `impl<T: Display> From<T> for Error` when propagated by `?` inside `FieldFuture::new` closures, silently dropping `extensions`. The new `to_graphql_error` returns `async_graphql::Error` directly so the extensions survive. Affects `UNIQUE_VIOLATION`, `NOT_NULL_VIOLATION`, `REFERENCES_VIOLATION`, `UNKNOWN_FIELD`, `TYPE_NOT_REGISTERED`, `CASCADE_CYCLE` — every code defined in PRD 00129 §6. (jink PRD 00022 blocker, PRD 00131)
