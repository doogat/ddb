...
- **server**: typed-mutation paths (`createDoogat`, `createMany`) now propagate `extensions.code = "UNIQUE_VIOLATION"` on duplicate-key violations through to the GraphQL error envelope, matching the contract documented in v0.2.5. The 0.2.5 `service::batch_create` refactor flattened the structured error to `DoogatError::Validation(String)` at the cross-batch (`validation.rs`) and intra-batch (`crud.rs`) conflict sites; both now return `DoogatError::unique_violation` so `to_server_error` re-attaches the code, columns, and values. `NOT_NULL_VIOLATION` is unchanged but now covered by a regression test alongside `UNIQUE_VIOLATION`. (jink PRD 00022 blocker, PRD 00131)
