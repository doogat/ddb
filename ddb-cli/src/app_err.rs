...
mod tests {
    ...
    fn cli_format_matches_legacy_for_validation() {
        ...
        let result = format_app_error(&AppError::from(DoogatError::Validation(
            "title required".into(),
        )));
    ...
    fn cli_format_matches_legacy_for_not_found() {
        ...
        let expected = format!("error: {}", de);
        let result = format_app_error(&AppError::from(DoogatError::NotFound(
            "doogat 999 missing".into(),
        )));
        assert_eq!(result, expected);
    }
    ...
    fn cli_format_matches_legacy_for_internal_git_error() {
        ...
        let expected = format!("error: {}", de);
        let result = format_app_error(&AppError::from(DoogatError::Git("repo corrupt".into())));
        assert_eq!(result, expected);
    }
}
