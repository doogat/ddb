...
const GITHUB_RELEASES_API: &str = "https://api.github.com/repos/doogat/ddb/releases/latest";
...
pub struct UpdateState {
    ...
    /// Version of the binary backed up at `config_dir()/ddb.previous`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
fn http_client() -> Result<reqwest::blocking::Client, String> {
    reqwest::blocking::Client::builder()
        ...
        .user_agent(format!("ddb/{CURRENT_VERSION}"))
        .build()
...
fn fetch_latest_release() -> Result<(Version, String), String> {
    ...
    let expected_name = format!("ddb-v{version}-{triple}.tar.gz");
    let asset = resp
        .assets
        .iter()
...
/// Spawn detached `ddb __update-check` process.
...
pub fn notify_if_updated() {
    ...
    if let Some(old_version) = state.updated_from.take() {
        eprintln!(
            "ddb updated v{old_version} -> v{}. restart your shell to use the new version.",
...
pub fn run_update() -> Result<(), String> {
    ...
    if latest <= current {
        eprintln!("ddb v{current} is already the latest version.");
        return Ok(());
    }

    eprintln!("downloading ddb v{latest}...");
    ...
    eprintln!("updated ddb v{current} -> v{latest}. restart your shell to use the new version.");
    Ok(())
}
...
fn download_and_replace(url: &str, expected_version: &Version) -> Result<(), String> {
    ...
    let tmp_dir = std::env::temp_dir().join(format!("ddb-update-{}", std::process::id()));
    ...
    for entry in archive.entries().map_err(|e| format!("tar error: {e}"))? {
        ...
        if name == "ddb" || name == "ddb.exe" {
            let dest = tmp_dir.join("ddb");
            entry
                .unpack(&dest)
    ...
    let binary_path = found_binary.ok_or("archive does not contain a `ddb` binary")?;
    ...
    self_replace::self_replace(&binary_path).map_err(|e| {
        format!(
            "self-replace failed: {e}. download manually from https://github.com/doogat/ddb/releases"
        )
    })?;
...
mod tests {
    ...
    fn state_round_trip() {
        let state = UpdateState {
            ...
            download_url: "https://example.com/ddb.tar.gz".into(),
            updated_from: None,
            previous_version: None,
        };
    ...
    fn state_read_write_with_temp_dir() {
        let dir = std::env::temp_dir().join(format!("ddb-test-{}", std::process::id()));
        let _ = fs::create_dir_all(&dir);
    ...
    fn backup_copies_file_to_dest() {
        let dir = std::env::temp_dir().join(format!("ddb-backup-test-{}", std::process::id()));
        let _ = fs::create_dir_all(&dir);

        let src = dir.join("ddb-test-bin");
        let dest = dir.join("ddb.previous");
    ...
    fn backup_path_under_config_dir() {
        let bp = backup_path();
        assert!(bp.ends_with("ddb.previous"));
        assert!(bp.parent().unwrap().ends_with("ddb"));
    }
